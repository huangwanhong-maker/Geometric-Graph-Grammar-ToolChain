...
def learn(
    ...
    *,
    ...
    which can be very slow on large inputs). Each level encodes the single best rule (see ``prefer``
    in ``selection.best_rule``: ``"gain"`` = max compression, ``"dense"`` = prefer closed shapes
    like triangles/squares), rewrites the graph, and repeats to build hierarchy.
    """
    g = graph.copy()
    rules = []
    rule_id = 0
    for level in range(max_levels):
        isos = detect_isogroups(
            ...
            frequency_cut=frequency_cut, params=params,
        )
        sel = best_rule(isos, prefer=prefer)
        if sel is None:  # nothing repeats enough to compress
            break
        iso, cover = sel
        rule = encode_rule(g, iso, cover, rule_id=rule_id, level=level, placement=placement)
        rules.append(rule)
        rule_id += 1
...
def _main(argv: list[str] | None = None) -> int:
    ...
    detect_kwargs = dict(
        ...
        frequency_cut=args.frequency_cut,
    )
    grammar = learn(
        graph, max_levels=args.max_levels, placement=args.placement,
        prefer=args.prefer, **detect_kwargs,
    )
    ...
    if args.draw:
        ...
        save_png(graph, args.draw, groups=groups)
